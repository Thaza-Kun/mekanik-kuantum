...
**POSTULAT 2**: Jumlah tenaga, $\varepsilon_n$, yang dipancarkan atau diserap oleh pengayun-pengayun dalam jasad hitam ialah hasil darab integer, $$\varepsilon_n = n\varepsilon,\ n=1,2,3,\cdots$$
...
```{proof, name="Kaedah Memperoleh Persamaan Planck"}
...
**USUL 1**: Pengayun-pengayung Planck mempunyai tenaga, $\varepsilon_n$, yang berkadaran terus terhadap frekuensi, $f$, dan berpekali integer, $n$, seperti yang dijelaskan dalam dua postulat Planck,
\begin{equation}
\varepsilon_n = nhf
(\#eq:mu-10)
\end{equation}
...
Jangan pula keliru antara $N$ dengan $n$. $N$ merujuk kepada jumlah pengayun manakala $n$ merujuk kepada jumlah ketulan cahaya.

Oleh itu, purata tenaga, $\overline{\varepsilon}$, yang dipancarkan oleh jasad hitam boleh diluahkan sebagai hasil jumlah tenaga yang dibawa oleh semua pengayun, $N(n)\varepsilon_n$, dibahagikan dengan jumlah pengayung, $N(n)$,
\begin{equation}
...
Nilai $N_0$ atas dan bawah boleh dipotong kerana ia hanyalah pemalar. Nilai $hf$ pula boleh difaktorkan keluar. Pemalar $\frac{-hf}{k_BT}$ dalam $\exp\Big\{\frac{-nhf}{k_BT}\Big\}$ akan digantikan dengan $k$. Pers. \@ref(eq:mu-12) kini adalah,
$$\overline{\varepsilon} = hf\frac{\sum n e^{kn}}{\sum e^{kn}}$$

===========================

Kini, kita boleh tulisnya sebagai hasil penjumlahan $n$,
\begin{equation}\label{6}
\overline{\varepsilon} = \frac{(0)hfx^0 + (1)hfx^1 + (2)hfx^2 + (3)hfx^3+\dots+(\infty)hfx^\infty}{x^0+x^1+x^2+x^3+\dots+x^\infty}.
\end{equation}

Dengan memfaktorkan keluar sebutan sepunya, kita dapat
\begin{equation}\label{6-1}
\overline{\varepsilon} = hfx \frac{\lim_{N\to\infty}1+2x+3x^2+\dots+Nx^{N-1}}{1+x+x^2+x^3+\dots+x^\infty}.
\end{equation}
Nilai pembawah itu didapati adalah suatu siri geometri. Nilai pengangka pula adalah pembezaan siri geometri.

\begin{petua}[Siri Geometri]
  \label{geometri}
Jujukan geometri ialah satu jujukan yang meningkat nilainya sebanyak satu pekali. Dalam jujukan $a, ax, ax^2, ax^3,...,ax^k$ setiap sebutan seterusnya didarabkan dengan $x$. Siri geometri pula adalah penjumlahan setiap sebutan dalam jujukan ini. Jika $-1<x<1$ dan $k\to\infty$, maka siri geometrinya adalah begini,
\begin{equation*}
    \lim_{k\to\infty}\sum_{n=0}^k ax^n = a\frac{1}{1-x}.
\end{equation*}
\margintext{\protect\vspace{-0.5cm}Ingatlah bahawa $$e^{-n} = \frac{1}{e^n}$$ maka ia tidak akan melebihi satu.}Memandangkan $x^n$ dalam pers. \ref{6-1} merujuk kepada $e^{-n\frac{hf}{k_BT}}$, maka kita sudah cukup syarat untuk menggunakan petua ini kerana $-1<e^{-n\frac{hf}{k_BT}}<1$. Ganti sahaja $a=1$.$\qed$
\end{petua}

\begin{petua}[Contoh 12.18, \citet{Mayer2006}]
Pembezaan siri geometri terhadap $n$ adalah
\begin{equation*}
\dv{x}\sum_{n=0}^{k}ax^n = \sum_{n=0}^{k} anx^{n-1},
\end{equation*}
dengan jumlahnya ialah
\begin{equation*}
\lim_{k\to\infty}\sum_{n=0}^{k} anx^{n-1} = a\frac{1}{(1-x)^2}.
\end{equation*}
dan gantilah $a=1$ untuk tujuan kita kali ini.$\qed$
\end{petua}

Lalu, kita akan peroleh
\begin{align}
\overline{\varepsilon} &= hfx\frac{(1-x)^{1}}{(1-x)^{2}}\,,\nonumber\\
&= hf\frac{x}{1-x}\,,\nonumber\\
&= hf\frac{1}{\frac{1}{x}-1}.\label{8}
\end{align}

Sepertimana yang dilakukan dalam penerbitan persamaan Rayleigh--Jeans, nilai $\overline{\varepsilon}$ ini akan didarabkan dengan Nilai Jeans (pers. \ref{0}). Nilai $x$ pula boleh digantikan dengan takrifan asalnya. Kita akan dapati persamaan ini adalah benar:
\begin{equation}\label{9}
U = n(f)\overline{\varepsilon} = \frac{8\pi hf^3}{c^3}\frac{1}{e^{\frac{hf}{k_BT}}-1}
\end{equation}
Lalu, kita ungkapkannya pula dalam bentuk panjang gelombang $\lambda$. Maka apa yang diperoleh adalah persamaan Planck,
